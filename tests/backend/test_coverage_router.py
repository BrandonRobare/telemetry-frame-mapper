...
def test_create_target_area(client):
    body = {"name": "Field A", "geom_geojson": '{"type":"Polygon","coordinates":[[[0,0],[1,0],[1,1],[0,1],[0,0]]]}'}
...
def test_delete_target_area(client):
    body = {"name": "ToDelete", "geom_geojson": '{"type":"Polygon","coordinates":[[[0,0],[1,0],[1,1],[0,0]]]}'}
...
def test_coverage_run_no_footprints(client):
    body = {"name": "Empty", "geom_geojson": '{"type":"Polygon","coordinates":[[[0,0],[1,0],[1,1],[0,1],[0,0]]]}'}
    area_id = client.post("/target-areas", json=body).json()["id"]
