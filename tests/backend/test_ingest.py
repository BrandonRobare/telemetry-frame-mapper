...
def make_gps_jpeg(lat: float, lon: float, alt_m: float, out_path: str, alt_ref: int = 0):
...
def test_extract_exif_with_gps_altitude_ref():
    """GPSAltitudeRef=1 makes altitude negative (below sea level)."""
    fd, path = tempfile.mkstemp(suffix=".jpg")
    os.close(fd)
    make_gps_jpeg(41.153, -81.341, 60.96, path, alt_ref=1)
    data = extract_exif(path)
    assert data["latitude"] == pytest.approx(41.153, abs=0.001)
    assert data["longitude"] == pytest.approx(-81.341, abs=0.001)
    assert data["altitude_m"] == pytest.approx(-60.96, abs=0.5)
