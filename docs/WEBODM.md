...
`exports_dir` at `webodm/<project_id>/<task_id>/`. It does not create a local queue or retain
WebODM credentials.
...
1. Import a session with at least two usable image files.
2. `POST /webodm/sessions/{session_id}/tasks` with optional `project_name`, `task_name`, and
...
3. Poll `GET /webodm/projects/{project_id}/tasks/{task_id}`. It maps WebODM statuses `10`, `20`,
   `30`, and `40` to queued, running, failed, and completed, including progress/error fields.
4. If needed, `POST /webodm/projects/{project_id}/tasks/{task_id}/cancel`; WebODM accepts the
   cancellation asynchronously.
5. Once status is completed, `POST /webodm/projects/{project_id}/tasks/{task_id}/results`.
