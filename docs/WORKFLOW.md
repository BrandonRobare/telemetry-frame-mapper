...
**Prerequisites:** everything in [INSTALL.md](INSTALL.md) — at minimum Python 3.11+, Node 22 LTS (>=22.12), `ffmpeg`, `exiftool`; plus COLMAP and an NVIDIA GPU for reconstruction. When anything fails, check [TROUBLESHOOTING.md](TROUBLESHOOTING.md) first — most errors in this pipeline are missing-binary or path-rule issues with exact known fixes.
