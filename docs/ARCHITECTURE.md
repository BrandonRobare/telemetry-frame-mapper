...
2. *COLMAP* (subprocess): `feature_extractor → exhaustive_matcher → mapper → model_converter` — the sparse model lands in `sparse/0/` in both BIN and TXT form. Per-image reprojection errors are parsed from the TXT model and surfaced in the Review tab. A similarity geo-transform (COLMAP space ↔ UTM) is solved from per-image camera centres against their EXIF GPS (Umeyama fit plus one >3σ residual trim) and stored as JSON on the record — this is what lets the viewer do GPS-accurate annotations and measurements. Degenerate geometry (fewer than three correspondences, near-zero spread, a near-collinear flight line) leaves `geo_transform` **NULL**; there is deliberately no identity placeholder, so every consumer must treat NULL as "not georeferenced" rather than silently emitting untransformed coordinates. `GET /reconstruction/{id}/calibration-drift-report` compares multiple completed COLMAP camera estimates when available; it returns `unavailable` for the normal single-camera result and never treats EXIF metadata as self-calibration output.
...
- **Heavy Python deps are lazy** — torch/gsplat/SuGaR/laspy import inside functions; `uv sync --group backend` and backend import must always work without them, degrading per the state machine above. Dependency groups are source-only; the wheel is CLI-only.
