...
4. *Lazy derivatives on first request:* LAS point cloud (laspy), DSM/ground-classified DEM GeoTIFF (laspy + rasterio), coverage-gap voxelization, voxel diff for Compare — each cached to `exports/{id}/` with its path stored on the record. DEM output refuses point clouds without ASPRS ground labels instead of inventing bare-earth terrain.
