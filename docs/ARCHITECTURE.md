...
Thirteen tabs: **Overview** (pipeline status and import CTA), **Map** (footprints + coverage on satellite imagery), **GPS Sync** (flight-log matching), **Review** (quality flags, frame selection, reprojection-error badges), **Plan** (target areas, lawnmower plans), **Export** (WebODM georeferencing CSV-only zip, GeoJSON, LAS, mesh), **Session Log** (event history + battery/flight records), **Reconstruct** (presets, job start), **Jobs** (resource monitor + job logs), **Storage** (disk breakdown + file browser), **Splat Viewer** (3D canvas, sparklines, coverage-gap heatmap, annotations, measurements, ortho/3D split, flythrough), **Compare** (voxel change detection between reconstructions), **Settings** (app, storage, mission, reconstruction, and render preferences).
