...
def _load_cuda_runtime():
    ...
    return torch, _backend


def _probe_cuda_runtime(torch, gsplat_backend) -> bool:
    """Return whether CUDA and gsplat's compiled extension are both usable."""
    try:
        return bool(torch.cuda.is_available() and gsplat_backend._C is not None)
...
def is_available() -> bool:
    """Cache the real CUDA/gsplat usability check for capability consumers."""
    try:
        torch, gsplat_backend = _load_cuda_runtime()
    except (ImportError, OSError, RuntimeError):
        return False
    return _probe_cuda_runtime(torch, gsplat_backend)
...
class CudaGsplatBackend:
    """The existing in-repository trainer driven by CUDA gsplat primitives."""
    ...
    def train(
        ...
        return train_splats(colmap_dir, output_path, config, progress_cb, cancel)
